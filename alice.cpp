...
{
    std::ifstream fin("P_A.txt");
    fin >> n_p;
    std::vector<std::vector<long>> R(n_p);
    long temp;
    for (int i = 0; i < n_p; i++)
    {
        for (int j = 0; j < 5; j++)
...
{
    std::ifstream fin("D_A.txt");
    fin >> n_d;
    std::vector<std::vector<long>> R(n_d);
    long temp;
    for (int i = 0; i < n_d; i++)
    {
        for (int j = 0; j < 2; j++)
...
{
    std::ifstream fin("Randoms_A.txt");
...
{
    std::ifstream fin("AlphaRandoms_A.txt");
    std::istream_iterator<long> start(fin), end;
    std::vector<long> numbers(start, end);
...
{
    static std::ifstream pin("Permutations_PA.txt");
    static std::ifstream din("Permutations_DA.txt");
    long temp;
    Pi_P.clear();
    Pi_D.clear();
    for(int i=0; i < n_p; i++)
    ...
    // {
    //     pin.close();
    //     pin.open("Permutations_PA.txt");
    // }
    // if(din.eof())
    // {
    //     din.close();
    //     din.open("Permutations_DA.txt");
    // }
}

void Alice::LoadNextShuffleVectors()
{
    static std::ifstream upin("U_PA.txt");
    static std::ifstream udin("U_DA.txt");
    static std::ifstream rpin("R_PA.txt");
    static std::ifstream rdin("R_DA.txt");
    static std::ifstream pipin("Pi_diff_R_PB_PA.txt");
    static std::ifstream pidin("Pi_diff_R_DB_DA.txt");
...
{
    static std::ofstream fout("Debug_A.txt");
    static int count = 0;
    ...
    // srv->Send(msg_for_Bob);
    // fout<<"Alice: Msg sent to Bob next_pc': "<<msg_for_Bob.scalar1 << "\n";
    // msg_from_Bob = srv->Receive();
    // fout<<"Alice: Msg rcvd from Bob next_pc\": "<<msg_from_Bob.scalar1 << "\n";
    // pc += msg_from_Bob.scalar1;
    // fout<<"Alice: next_pc = "<<pc<<"\n";
    // if(pc >= n_p || pc < 0){
    //     P.clear();
    //     D.clear();
    //     LoadProgramShare();
...
double Alice::ObliviousMultiplication(long arg1, long arg2)
{
    // long result;
    // Message msg_for_Bob, msg_from_Bob;
    // msg_for_Bob.scalar1 = arg1;
    // msg_for_Bob.scalar2 = arg2;
    // srv->Send(msg_for_Bob);
    // msg_from_Bob = srv->Receive();
    // result = (arg1+msg_from_Bob.scalar1)*(arg2+msg_from_Bob.scalar2);
    // result -= 100;
...
{
    static int count=0;
    static std::ofstream fout("DebugTranslate_A.txt");
    for(int i = 0; i < n_p; i++)
    {
