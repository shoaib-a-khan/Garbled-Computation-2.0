...
void Helper::GenerateAlphaRandoms(int n)
{
    ...
    std::vector<long> R(n);
    time_t t;
    fa.open("AlphaRandoms_A.txt");
    fb.open("AlphaRandoms_B.txt");
    srand((unsigned)time(&t));
    for (int i = 0; i < n; i++)
        R[i] = rand() % 10000;
    for (int i = 0; i < n; i++)
...
void Helper::GenerateRandomPermutations(int n_p, int n_d, int count)
{
    ...
    std::ofstream output_file_pa("./Permutations_PA.txt");
    std::ostream_iterator<int> output_iterator_pa(output_file_pa, " ");
    std::ofstream output_file_da("./Permutations_DA.txt");
    std::ostream_iterator<int> output_iterator_da(output_file_da, " ");
    std::ofstream output_file_pb("./Permutations_PB.txt");
    std::ostream_iterator<int> output_iterator_pb(output_file_pb, " ");
    std::ofstream output_file_db("./Permutations_DB.txt");
...
void Helper::GenerateShuffleVectors(int n_p, int n_d, int count)
{
    ...
    std::vector<std::string> file_name = {"U_PA.txt", "U_PB.txt", "R_PA.txt", "R_PB.txt",
                                          "U_DA.txt", "U_DB.txt", "R_DA.txt", "R_DB.txt"};
    time_t t;
    srand((unsigned)time(&t));
...
void Helper::GeneratePermutedVectors(int n_p, int n_d, int count)
{
    ...
    std::vector<std::string> file_name = {"Pi_diff_R_PA_PB.txt", "Pi_diff_R_PB_PA.txt",
                                          "Pi_diff_R_DA_DB.txt", "Pi_diff_R_DB_DA.txt",
                                          "R_PA.txt", "R_PB.txt", "R_DA.txt", "R_DB.txt"};
    for (int i = 0; i < 4; i++)
    {
        output_file.open(file_name[i]);
        int n = (i < 2 ? n_p : n_d);
...
{
    static std::ifstream pin_a("Permutations_PA.txt");
    static std::ifstream pin_b("Permutations_PB.txt");
    static std::ifstream din_a("Permutations_DA.txt");
    static std::ifstream din_b("Permutations_DB.txt");
    std::vector<int> Pi;
    long temp;
